...
EXTERNAL_FORWARD_FROM_GROUP_ID=-100...
INTERNAL_FORWARD_FROM_GROUP_ID=-100...
ADMIN_ID = [543583405]
...
class BroadcastBot:
    ...
    async def set_internal_forward_limit_handler(self, update, context):
        ...
        await query.answer()
        await query.edit_message_text(
            "Введите число — сколько последних сообщений пересылать (1-20):",
            reply_markup=self._create_cancel_button()
        )
                ...
                if mode == "message":
                    ...
                    for msg in last_messages:
                        try:
                            ...
                            print(f"Ошибка при пересылке сообщения {msg.id} -> {user_id}: {e}")
            except FloodWait as e:
                await asyncio.sleep(e.value)
                try:
                    await client.send_message(user_id, message)
    ...
    async def external_broadcast_confirm_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if mode == "message":
            ...
            preview = f"Выбрана пересылка сообщений"
    ...
    async def _external_broadcast_worker(self, config, userbot_clients, update, context):
        ...
        if mode == "links":
            last_messages = []
            async for msg in userbot_clients[0].get_chat_history(EXTERNAL_FORWARD_FROM_GROUP_ID, limit=2):
                last_messages.append(msg)
            last_messages = list(reversed(last_messages))
        else:
            last_messages = []
